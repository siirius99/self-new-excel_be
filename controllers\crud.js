...
module.exports = function (app) {
  ...
  controller.get_schema = async function (req, res, next) {
    try {
      ...
      let __tenant_table = null;
      let __tenant_primary_key = null;
      const __table_details = _.find(_.get(app.mysql_schema, 'tables'), (item) => item.tenant);
      if (__table_details) {
        __tenant_table = __table_details.name
        __tenant_primary_key = _.findKey(_.get(__table_details, 'external_fields') || {}, { primary: true });
      }
      if (__user_id !== '-1') {
        for (const __t of __tables) {
          if (__t.tenant) {
            __t.crud_get_enabled = __user_role === _.get(app.constants, ['roles', 'OWNER']) ? __t.crud_get_enabled : false;
            __t.crud_post_enabled = false;
            __t.crud_delete_enabled = false;
            __t.crud_soft_delete_enabled = false;
            __t.crud_put_enabled = __user_role === _.get(app.constants, ['roles', 'OWNER']) ? __t.crud_put_enabled : false;
          }
          if (__t.member) {
            __t.crud_get_enabled = __user_role === _.get(app.constants, ['roles', 'OWNER']) ? __t.crud_get_enabled : false;
            __t.crud_post_enabled = __user_role === _.get(app.constants, ['roles', 'OWNER']) ? __t.crud_post_enabled : false;
            __t.crud_delete_enabled = __user_role === _.get(app.constants, ['roles', 'OWNER']) ? __t.crud_delete_enabled : false;
            __t.crud_soft_delete_enabled = __user_role === _.get(app.constants, ['roles', 'OWNER']) ? __t.crud_soft_delete_enabled : false;
            __t.crud_put_enabled = __user_role === _.get(app.constants, ['roles', 'OWNER']) ? __t.crud_put_enabled : false;
          }
          if(__tenant_table && __tenant_primary_key && __user_role && __user_role !== _.get(app.constants, ['roles', 'OWNER'])){
            for(let __c of __t.columns){
              if(_.get(__t, ['external_fields', __c, 'reference', 'table']) === __tenant_table && _.get(__t, ['external_fields', __c, 'reference', 'column']) === __tenant_primary_key){
                _.set(__t, ['external_fields', __c, 'session'], true)
              }
            }
          }
        }
  ...
  controller.login = async function (req, res, next) {
    try {
      ...
      if (__email === __root_email) {
        ...
        const authDetails = await app.libs.auth.generate_token({ id: '-1', role: _.get(app.constants, ['roles', 'OWNER']) });
        return res.jsonp(
          _.merge(authDetails, {
      ...
      if (memberTableDetails) {
        ...
        __tenant_primary_key = _.findKey(_.get(memberTableDetails, ['external_fields'], {}), { primary: true });
        if (!__tenant_column) {
          throw new app.libs.customError('Incorrect username or password', 403);
        }
      }
      ...
      const isPasswordMatch = await app.libs.crud.compare_password({
        password: __password,
      ...
      if (isPasswordMatch) {
        ...
        const authDetails = await app.libs.auth.generate_token({
          ...
          member_id: memberTableDetails ? result[0][__tenant_primary_key] : null,
        });
        return res.jsonp(
          _.merge(authDetails, {
