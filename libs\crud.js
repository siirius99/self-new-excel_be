...
module.exports = function (app) {
    ...
    return new Promise(async function (resolve, reject) {
      try {
        ...
        if(__tenant_table && __tenant_primary_key && __user_role && __user_role !== _.get(app.constants, ['roles', 'OWNER'])){
          for(let __c of __crud_details.columns){
            if(_.get(__crud_details, ['external_fields', __c, 'reference', 'table']) === __tenant_table && _.get(__crud_details, ['external_fields', __c, 'reference', 'column']) === __tenant_primary_key){
              __optional_fields.push(__c)
            }
          }
        }
        ...
        await app.libs.validation.joi_validation_v1({
          schema: await app.libs.defined_joi_schema.get_schema({
            ...
            optional_fields: __optional_fields
          }),
          data: options.body,
        });
  ...
  wrapper.compare_password = function (options) {
    return new Promise(async function (resolve, reject) {
      try {
        return resolve(await bcrypt.compare(options.password, options.password_hash));
      } catch (e) {
        return app.libs.error_handler.non_controller_handler({
          e,
          reject,
