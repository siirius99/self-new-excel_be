...
module.exports = function (app) {
  ...
  utils.get_basic_insert_details = function (options) {
    ...
    const __user_role = _.get(options.user_info, 'user.role');
    ...
    if(__tenant_table && __tenant_primary_key && __user_role && __user_role !== _.get(app.constants, ['roles', 'OWNER'])){
      for (const __column in __field_details) {
        if (_.get(__field_details, [__column, 'reference', 'table']) === __tenant_table && _.get(__field_details, [__column, 'reference', 'column']) === __tenant_primary_key) {
          __to_return[__column] = _.get(options, ['user_info', 'user', 'id']) || -1;
        }
      }
    }
  ...
  utils.sync_and_update_password = function (options) {
    return new Promise(async function (resolve, reject) {
      try {
        const __user_id = _.get(options, ['user_info', 'user', 'id'], 'NA');
        const __body = await app.libs.crud.encrypt_user_password({
        ...
        const __password_match = await app.libs.crud.compare_password({
          ...
          password_hash: _.get(options, 'old_password_hash'),
          user_info: options.user_info,
        });
        if (__password_match) {
          throw new app.libs.customError('Can not set old password again', 409);
        }
        if (_.size(options.member)) {
          const __member_id = _.get(options, ['user_info', 'member', 'id'], 'NA');
          await app.db.update_record_by_ID_or_UUID({
            table_name: options.member.table_name,
            id: __member_id,
            identifier: options.member.primary_key,
            update: _.merge(
              __body,
        ...
        await app.db.update_record_by_ID_or_UUID({
          table_name: options.tenant.table_name,
          id: __user_id,
          identifier: options.tenant.primary_key,
          update: _.merge(
            __body,
            app.libs.utils.get_basic_insert_details({ meta: { update: true }, user_info: options.user_info })
          ),
        ...
        return app.libs.error_handler.non_controller_handler({
          e,
          reject,
          function_name: 'sync_and_update_password',
          options,
        });
      }
    });
