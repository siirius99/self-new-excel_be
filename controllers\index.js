...
module.exports = function (app) {
  ...
  controller.register_user = async function (req, res, next) {
    try {
      ...
      if (!__table_details) {
        throw new app.libs.customError('Registration is currently closed. We apologize for any inconvenience.', 400);
      }
      ...
      let __tenant_primary_key = null;
      let __member_result = null;
      if (__member_table_details) {
        ...
        __tenant_primary_key = _.findKey(_.get(__member_table_details, ['external_fields'], {}), { primary: true });
        if (__tenant_column) {
          ...
          __member_result = await app.db.records(_.cloneDeep( { table_name: __member_table_details.name, email: _.get(req.body, 'email') }));
        }
      }
      const authDetails = await app.libs.auth.generate_token({
        id: _.get(__result, [0, __primary_key]),
        role: _.get(app.constants, ['roles', 'OWNER']),
        member_id: __member_table_details ? __member_result[0][__tenant_primary_key] : null,
      });
      return res.jsonp(
        _.merge(authDetails, {
  ...
  controller.change_user_password = async function (req, res, next) {
    try {
      await app.libs.validation.joi_validation_v1({
        schema: Joi.object().keys({
      ...
      const __table_details = _.find(_.get(app.mysql_schema, 'tables'), (item) => item.tenant);
      if (!__table_details) {
        throw new app.libs.customError('Change password is currently closed. We apologize for any inconvenience.', 400);
      }
      const __primary_key = _.findKey(_.get(__table_details, 'external_fields') || {}, { primary: true });
      if (!__primary_key) {
        throw new app.libs.customError('Change password is currently closed. We apologize for any inconvenience.', 400);
      }
      __tenant.table_name = __table_details.name;
      __tenant.primary_key = __primary_key;
      const __member_table_details = _.find(_.get(app.mysql_schema, 'tables'), (item) => item.member);
      let __where_cl = { table_name: __table_details.name, [__primary_key]: _.get(req.user_info, ['user', 'id']) };
      if (__member_table_details) {
        const __member_id = _.get(req, ['user_info', 'member', 'id'], 'NA');
        let __tenant_column = null;
        for (const __c in _.get(__member_table_details, ['external_fields'], {})) {
          const __cr_meta = _.get(__member_table_details, ['external_fields', __c, 'reference'], {});
          if (
            !__tenant_column &&
            _.get(__cr_meta, ['table']) === __table_details.name &&
            _.get(__cr_meta, ['column']) === __primary_key
          ) {
            __tenant_column = __c;
            break;
          }
        ...
        if (__tenant_column) {
          __member.table_name = __member_table_details.name;
          __member.primary_key = _.findKey(_.get(__member_table_details, ['external_fields'], {}), { primary: true });
        }
        __where_cl = { table_name: __member_table_details.name, [__member.primary_key]: __member_id };
      }
      let __result = await app.db.records(_.cloneDeep(__where_cl));
      if (__result.length === 0) {
        throw new app.libs.customError('Invalid request', 400);
      }
      [__result] = __result;
      const __password_match = await app.libs.crud.compare_password({
        password: _.get(req, 'body.old_password'),
        password_hash: __result.password,
        user_info: req.user_info,
      });
      if (!__password_match) {
        throw new app.libs.customError('Old password does not match', 400);
      }
      await app.libs.utils.sync_and_update_password({
        user_info: req.user_info,
        old_password_hash: __result.password,
        tenant: __tenant,
        member: __member,
        password: _.get(req, 'body.password'),
      });
      return res.jsonp({ message: 'Password changed successfully' });
    } catch (e) {
      return app.libs.error_handler.controller_handler({
        e,
        res,
